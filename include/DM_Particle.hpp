...
//1. DM-nucleus scattering cross sections:
	...
	class DM_Particle
	{
		private:
			...
			double FormFactor_Atay(double q,int Z)const;
		public:
			bool ldm;
			...
			//Constructor
			DM_Particle();
			DM_Particle(double mDM,double sn,double se=0.0,bool light=true,std::string ff="Contact",std::string za="A",bool scr=false,double mMed=0.0, double Qcharge=0.0);
			
			void Set_Mass(double m);
			void Set_Charge(double QCharge);
			void Set_LDM(bool IsItLDM);
...
//2. DM Speed distribution
	...
	// extern double dfdvE(double v,double vEarth,int npower);
	// extern double dfdvesc(double v,double vEarth);
	// extern double dfdv(double v,double vEarth);
