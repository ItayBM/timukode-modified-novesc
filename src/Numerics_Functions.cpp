	...
	//Function to return a reasonable precision.
		double Find_Epsilon(std::function<double(double)> func, double a,double b,double precision)
		{
			...
			double h = b - a;                                                                  
				...
				// epsilon=min(epsilon,h/1000.0);
				return epsilon;
		}
	//Just a trapz basically....
		double ItayIntegrate(std::function<double(double)> func, double xMin,double xMax, double dx)
		{
			double integral=0.0;
			double x=xMin;
			while(x<xMax)
		...
		double Integrate(std::function<double(double)> func, double a,double b, double epsilon,int maxRecursionDepth)
		{
			...
			double h = b - a;                                                                  
				...
				if(warning)   
				{
					std::cout <<"Warning in Integrate(): Numerical integration on the interval ("<<a<<","<<b<<") with epsilon "<<epsilon<<" did not converge to the desired precision." <<std::endl;
...
//4. One-dimensional interpolation of tabulated data using Steffen splines 
	//Computation of the coefficients
		...
		unsigned int Interpolation::Locate(double x)
		{
			...
			{
				if(((xDomain[0]-x)/(xDomain[1]-xDomain[0])<0.0001) && ((x-xDomain[1])/(xDomain[1]-xDomain[0])<0.0001))
				{
					// printf("\nWarning in Interpolation::Locate(): x = %e lies at the edge of the domain [%e,%e].\n\n",x,xDomain[0],xDomain[1]);
					if(xDomain[0]-x>0.0)
					{
						x=xDomain[0];
					}
