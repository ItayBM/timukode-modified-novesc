...
////////////////////////////////////////////////////////////
	...
	if(argc > 2)
	{
		if(argc == 3 || argc > 9)
		{
			cerr <<"Error in main(): For PDF-only run, DaMaSCUS-CRUST requires between 2 and 4 additional command line arguments: mDM[GeV], Q[] (optional: vMin[km/sec], SampleSize)."<<argc<<"."<<endl;
			std::exit(EXIT_FAILURE);
		}
		double mDM_pdf = atof( argv[2]) * GeV;
		double charge_pdf=atof(argv[3]);//Itay addition! now it reads the charge, and computes sigma pdf
		double sigma_pdf = cm*cm*pow(mDM_pdf,2.0)*pow(charge_pdf,2.0)*4.746*pow(10.0,-9.0)/pow(mDM_pdf+mNucleon,2.0);//Itay change to conform with the previous line
		// cout <<"reconstructing sigma_pdf="<<sigma_pdf<<"."<<endl;
		// cout <<"reconstructing Q="<<charge_pdf<<"."<<endl;
		// cout <<"reconstructing mDM="<<mDM_pdf<<"."<<endl;
		// cout <<"reconstructing mNuc="<<mNucleon<<"."<<endl;
		double vMin_pdf = (argc>4)? atof(argv[4]) * km/sec : 0;
		if(argc>6 && argc < 8)
			{
			cerr <<"Error in main(): For PDF-only run, DaMaSCUS-CRUST requires between 2 and 4 additional command line arguments: mDM[GeV], Q[] (optional: vMin[km/sec], SampleSize) and then add v0,vearth,VESC."<<endl;
			std::exit(EXIT_FAILURE);
			}
		if(argc>6)
			{
				...
				vEarth = atof(argv[7]);
				v0=v0*km/sec;
				vEarth=vEarth*km/sec;
				Nesc=M_PI*v0*v0*sqrt(M_PI)*v0;
			}
		// cout <<"\t\tv0:\t\t" <<InUnits(v0,km/sec)<<" km/sec" <<endl;
		// cout <<"\t\tvE:\t\t" <<InUnits(vEarth,km/sec)<<" km/sec" <<endl;
		if(argc>5) SampleSize = atof(argv[5]);
		DM.Set_Mass(mDM_pdf);
		DM.Set_Charge(charge_pdf);
		DM.Set_Sigma_n(sigma_pdf);
		double mutemp=Reduced_Mass(DM.mass,28.0*mNucleon);
		if(FormFactor_N(2*mutemp*min(vEarth+3*v0,vEarth+3.0*v0),14.0,true)>0.8) DM.Set_LDM(true);
		// cout<< "LDM is "<<DM.ldm<<"."<<endl;
		double Analytic_vMean = (vEarth>100.0001234234*v0)? vEarth : Average_Speed(vEarth,vMin_pdf);
		...
		if(myRank == 0)
		{
			...
			//1. MC result
				...
				// cout<<min(Global_Data)<<endl;
				Interpolation kde = Perform_KDE(Global_Data,vMin_pdf,(3*v0+vEarth));
				kde.Multiply(attenuation[0]);
				//1.3 Save the PDF to a file
				std::ostringstream streamObj;
				streamObj <<"../results/"<<ID <<"/DM_Speed_PDF_mDM="<< mDM_pdf<<"_GeV_Q="<<charge_pdf <<"_vMin="<<vMin_pdf/km*sec<<"_kps_v0="<<v0/km*sec<<"_kps_vEarth="<<vEarth/km*sec<<"_kps_vesc=infinity_kps_Thickness="<<Layers[0].thickness/km<<".txt";
