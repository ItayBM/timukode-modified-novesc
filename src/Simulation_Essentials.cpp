...
//3. Shielding layers
	...
	void Layer::Compute_MFP(const DM_Particle &DM,double vmin)
	{
		...
		{
			cout <<"Warning in Compute_MFP: you are using contact interactions. The Timukode assumes that the mfp is not velocity dependent. Not sure if that is true for semi-relativistic guys."<<endl;
			tabulated = false;
			prob.clear();
			mfp=0.0;
		...
		else
		{
			...
			double dv = (3*v0+vEarth-vMin)/(N-1);
			for(int i=0;i<N;i++)
			{
	...
	double Layer::MFP(double vDM)
	{
		if(tabulated)
		{
			...
			int i = std::floor( 1.0*(vDM-vMin)/(3*v0+vEarth-vMin)*N );
			return mfp_array[i];
		}
		else
		{
	...
	double Layer::Stopping_Power(const DM_Particle& DM, double vDM) const
	{
		...
		for(unsigned int i = 0;i<composition.size();i++)
		{
			...
			double mu=Reduced_Mass(DM.mass,mA);
			double ERmax = 2.0*pow(Reduced_Mass(DM.mass,mA)*vDM,2.0)/mA;
			if(((DM.ldm)||(FormFactor_N(2.0*mu*vDM,A,true)<0.8))&&(DM.formfactor!="General"))
			{
				double a = Thomas_Fermi_Radius(Z);
				if(DM.formfactor=="Contact")
				{
					if(!DM.screening)	stopping_power+=ERmax*nT*DM.sigmaSI(Z,A)/2.0;
					else
					{
				...
				else if(DM.formfactor=="Long-Range")
				{
					...
					//Itay addition. Relying on a fit to data from protons through SiO2
					// if(vDM<0.005)
					// {
					// stopping_power+= nT*pow(DM.charge,2.0)*60567.0*vDM*MeV*pow(cm,2.0)/gram;//60567*beta is the stopping power in MeV*cm^2/gram. 
					// }
					// else if(vDM<0.04)
					// {
					// stopping_power+= nT*pow(DM.charge,2.0)*exp(-6.19-5.68*log(vDM)-0.65*pow(log(vDM),2.0))*MeV*pow(cm,2.0)/gram;
					// }
					// else if(vDM<0.7)
					// {
					// stopping_power+= nT*pow(DM.charge,2.0)*exp(0.7-1.47*log(vDM))*MeV*pow(cm,2.0)/gram;
					// }
				}
			}
			else
	...
	std::vector<double> Layer::Sample_Target(double vDM,std::mt19937& PRNG)
	{
		...
		//Which element in the element?
			...
			for(i=0;i<composition.size();i++)
			{
				if(tabulated)
				{
					int j = std::floor( 1.0*(vDM-vMin)/(3*v0+vEarth-vMin)*N );
					sum+= prob_array[j][i];
				}
				else
				{
	...
	//Compute mfp for all existing layers:
		...
		{
			for(unsigned i = 0; i<Layers.size();i++) Layers[i].Compute_MFP(DM,vmin);
		}
	...
	//Event when particle leaves the current layer
		Event Leave_Layer(Event& x)
		{
			...
			//Find t_Exit
				...
				if(vz<0) zExit -= Layers[layer-1].thickness;
	...
	void Compare_StoppingPower(const DM_Particle& DM,double vDM,const std::vector<Layer>& layers,int rank)
	{
		if(layers.size()>1)
		{
			...
			if(rank ==0)
			{
				std::cout<<std::endl<<"Relative stopping power of the layers."<<std::endl;
				for(unsigned int i=0;i<layers.size();i++)
				{
	...
	std::vector<Layer> Atmospheric_Layers(int layers,double altitude)
	{
		...
		//Atmosphere composition
			...
			for(unsigned int i=0;i<f.size();i++) composition.push_back({f[i],Z[i],A[i]});
		...
		double Rejection_Sampling(const std::function<double(double)>& PDF,double xMin,double xMax,double yMax,std::mt19937& PRNG)
		{
			...
			while(!success)
			{
				...
				if(count%1000==0)
				{
					...
					// cout <<"Warning in Rejection_Sampling(): Very inefficient sampling with y="<<y<<"."<<endl;
					// cout <<"Warning in Rejection_Sampling(): Very inefficient sampling with x="<<x<<"."<<endl;
					// cout <<"Warning in Rejection_Sampling(): Very inefficient sampling with pdf="<<pdf<<"."<<endl;
					// cout <<"Warning in Rejection_Sampling(): Very inefficient sampling with yMax="<<yMax<<"."<<endl;
					// double v=x;
					// double term1=M_PI*v*v0*v0/Nesc/vEarth*(exp(-pow(v-vEarth,2)/v0/v0)-exp(-pow(v+vEarth,2)/v0/v0));
					// std::cerr <<"Error in Rejection_Sampling(): PDF is negative -> f("<<x<<")=" <<pdf<<endl;
					// std::cerr <<"First term: "<<term1<<"."<<endl;
					// std::cerr <<"2nd term: "<<term2<<"."<<endl;
					// std::cerr <<"3rd term: "<<term3<<"."<<endl;
				}
				if(count%100000==0)
				{
					cout <<"Error in Rejection_Sampling(): Very inefficient sampling with xMax="<<xMax<<"."<<endl;
					cout <<"Error in Rejection_Sampling(): Very inefficient sampling with xMin="<<xMin<<"."<<endl;
					cout <<"Error in Rejection_Sampling(): Very inefficient sampling with yMax="<<yMax<<"."<<endl;
					cout <<"Error in Rejection_Sampling(): Very inefficient sampling with N="<<count<<"."<<endl;
					std::exit(EXIT_FAILURE);
				}
				if (pdf < 0.0) 
				{
					double v=x;
					double term1=M_PI*v*v0*v0/Nesc/vEarth*(2*exp(-(v*v+vEarth*vEarth)/v0/v0)*sinh(2*v*vEarth/v0/v0));
					std::cerr <<"Error in Rejection_Sampling(): PDF is negative -> f("<<x<<")=" <<pdf<<endl;
					std::cerr <<"First term: "<<term1<<"."<<endl;
					//
					std::exit(EXIT_FAILURE);
				...
				else if(pdf>yMax)
				{
					...
					std::cout <<"ymax="<<yMax<<". PDF="<<pdf<<"."<<endl;
					std::cout <<"x="<<x<<"."<<endl;
...
//5. Functions for splitting/Russian roulette
	...
	{
		if(ProbabilitySample(PRNG)<pSurv) return true;
		else return false;
	}
	...
	{
		if(importances.empty()) return 1;
		else if(z>=importance_boundaries.front()) return 0;
		else if(z<=importance_boundaries.back()) return importances.back();
		else
		{
			unsigned int i;
			for(i=0;i<importance_boundaries.size();i++)
			{
				if(z>=importance_boundaries[i]) break;
			}
