...
//3. Direct detection recoil spectrum
	...
	{
		//0. maximal energy
			double ERmax = ERMax((3*v0+vEarth),DM.mass,A);
		...
		//2. Find the Kernel density estimate for the speed distribution function
			...
			Interpolation kde = Perform_KDE(speeddata,vCutoff,(3*v0+vEarth));
			// cout <<"IM here0.2"<<endl;
				...
				//3.2 Integrate.
					...
					else
					{
				  		...
				  		//NOTE: Gives precision warnings regularly due to a too small choice of epsilon.
				  			double epsilon = Find_Epsilon(integrand,vMin,(3*v0+vEarth),1.0e-3);
				  			// cout <<"IM here4"<<endl;
							// integral = ItayIntegrate(integrand,vMin,(3*v0+vEarth),v0/1000.0); 
							integral = Integrate(integrand,max(vMin,vEarth-3.0*v0),min(3*v0+vEarth,vEarth+3.0*v0),epsilon);
							// cout <<"IM here0.4"<<endl;
					}
				//3.3 Append to list 
