...
//			double ymax = (vCutoff>350*km/sec)? SpeedDistribution(vCutoff,vEarth) : 1000.0;
			double ymax = 2*M_PI*(vEarth+3*v0)*v0*v0/(Nesc*vEarth);
			// cout<<"ymax="<<ymax<<"."<<endl;
			auto pdf = std::bind(SpeedDistribution,_1,vEarth);
			// cout<<"pdf is "<<pdf(vEarth)<<"."<<endl;
			// cout<<"ymax is "<<ymax<<"."<<endl;
			double v = Rejection_Sampling(pdf,max(vCutoff,vEarth-3.0*v0),min(3*v0+vEarth,vEarth+3.0*v0),ymax,PRNG);
			// cout<<"NEXT."<<endl;
		//Velocity Direction
...
//Perform the scattering.
	double PDF_ScatterAngle(double cosalpha,const DM_Particle& DM,const std::vector<double>& target,double vDM,double deltaIS)
	{
		...
		if((!DM.ldm)&&(FormFactor_N(2.0*mu*vDM,A,true)<0.8))
		{
		...
		if((!DM.ldm)&&(FormFactor_N(2.0*mu*vDM,A,true)<0.8))
			{
				...
				cout<<"cosine? really"<<endl;
				double pdfmax=pdf(1.0);
				double xminloc=-1.0;
				double pdfmin=pdf(xminloc);
				while(pdfmin<pdfmax/200.0)
					{
						pdfmin=pdf(xminloc);
						xminloc=xminloc+(1-xminloc)/2;
					}
					if(pdfmin>pdfmax/50.0)
					{
						xminloc=max((-1.0 + 3.0* xminloc)/2.0,-1.0);
					}
				cosalpha = Rejection_Sampling(pdf,xminloc,1.0,pdfmax,PRNG);
				cout<<"cosine? huh..."<<endl;
